...
/**
 * Classe de dados para armazenar os resultados da equação do 2º grau e os dados do gráfico.
 * @param x1 Primeira raiz (pode ser nula se não houver raízes reais).
 * @param x2 Segunda raiz (pode ser nula se não houver raízes reais).
 * @param graphPoints Lista de DataPoint para desenhar a parábola.
 * @param graphMinX Limite mínimo do eixo X para o gráfico.
 * @param graphMaxX Limite máximo do eixo X para o gráfico.
 * @param graphMinY Limite mínimo do eixo Y para o gráfico.
 * @param graphMaxY Limite máximo do eixo Y para o gráfico.
 * @param resultText Texto formatado do resultado para exibição no TextView.
 */
data class EqGrau2Result(
    val x1: Double?,
    val x2: Double?,
...
class CoeficientesViewModel : ViewModel() {

    // LiveData para os coeficientes (entrada do usuário)
    ...
    private val _result = MediatorLiveData<EqGrau2Result>()
    val result: LiveData<EqGrau2Result> = _result
    ...
    init {
        ...
        _result.addSource(indiceA) {
            calculateAndSetResult(it, indiceB.value, indiceC.value)
        }
        _result.addSource(indiceB) {
            calculateAndSetResult(indiceA.value, it, indiceC.value)
        }
        _result.addSource(indiceC) {
            calculateAndSetResult(indiceA.value, indiceB.value, it)
        }

        // Realiza um cálculo inicial para que o gráfico não esteja vazio ao iniciar
        calculateAndSetResult(0.0, 0.0, 0.0) // Exemplo: gera uma linha y = 0
    }
    ...
    fun calculateAndSetResult(a: Double?, b: Double?, c: Double?) {
        ...
        if (delta >= 0) {
            ...
            x2 = (-b - sqrt(delta)) / (2 * a)
            resultText = "X1 = %.1f\nX2 = %.1f".format(x1, x2)
            _visor1Text.value = "X1 = ${x1.format(1)}; X2 = ${x2.format(1)}"
        } else {
            val rm = -b / (2 * a)
            val im = sqrt(-delta) / (2 * a)
            resultText = "X1 = %.2f + i(%.2f)\nX2 = %.2f - i(%.2f)".format(rm, im, rm, im)
            _visor1Text.value = "X1 = ${rm.format(2)} + i(${im.format(2)}); X2 = ${rm.format(2)} - i(${im.format(2)})"
        }
    ...
    // Funções para atualizar os valores dos coeficientes (chamadas da UI)
    ...
    // Helper para formatar Double (para números complexos)
    private fun Double.format(digits: Int) = String.format("%.${digits}f", this)

    /**
     * Limpa todos os dados e reseta o ViewModel para o estado inicial.
     */
    fun clearAll() {
        _indiceA.value = 0.0
        _indiceB.value = 0.0
    ...
    /**
     * Limpa apenas o texto dos visores.
     */
    fun clearVisorOnly() {
        _visor1Text.value = "0" // Reseta para "0"
        _visor2Text.value = "0.0*X^2 + (0.0)*X + 0.0 = 0" // Reseta para a equação padrão
